...
## Q8. REPL(`>>>`)에서 `print(Path(__file__).resolve().parent)`를 실행하면 왜 `NameError: name '__file__' is not defined`가 발생하나요?

**답변**

`__file__`은 **현재 실행 중인 `.py` 파일의 경로**를 담고 있는 특수 변수인데, 이건 파이썬이 **스크립트 파일을 읽어서 실행할 때만** 자동으로 만들어준다.

`>>>` 프롬프트는 대화형 인터프리터(REPL)인데, 여기서는 코드를 파일이 아니라 **한 줄씩 즉석에서 입력받아 실행**하는 것이라 "이 코드가 담긴 파일"이라는 게 애초에 존재하지 않는다. 그래서 `__file__`을 만들어줄 근거가 없어 `NameError`가 발생한다.
...
```python
# ex22.py 안에서는 __file__이 그 파일의 경로를 가짐
from pathlib import Path
print(Path(__file__).resolve().parent)   # 정상 출력됨
```

```python
# 반대로 REPL(>>>)에서는 파일이 없으므로 에러
>>> print(__file__)
