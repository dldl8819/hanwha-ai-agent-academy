...
# 현재 파일 기준으로 app/agent/prompts 폴더 경로 잡기
PROMPT_DIR = Path(__file__).parent / "prompts"

# md 파일을 읽어서 문자열로 리턴해주는 함수
def load_prompt(name: str) -> str:
    # name : 확장자를 제외한 파일명 "answer_system"
    path = PROMPT_DIR / f"{name}.md"
    if not path.is_file():
        raise FileNotFoundError(f"프롬프트 파일을 찾을 수 없습니다. : {path}")
    # 파일 읽은 내용 앞뒤 공백 없애고 리턴
    return path.read_text(encoding="utf-8").strip()

# system + human 2칸짜리 프롬프트 템플릿을 만들어주는 함수
def build_prompt(*, system_prompt: str | None = None) -> ChatPromptTemplate:
    # system_prompt : 시스템 메시지 본문 
    # - 외부에서 주지 않으면 answer_system으로 처리
...
# 질문을 LLMResult로 변형하는 함수
def build_result_chain(
        # llm : LLMPort를 만족하는 어댑터. 
        # - 추후 LLMPort 규격만 맞으면 다른 API 모델 사용 가능
        llm: LLMPort,
        *,
